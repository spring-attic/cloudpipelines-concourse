...
** https://github.com/CloudPipelines/concourse[`Cloud Pipelines Concourse`]
** https://github.com/CloudPipelines/scripts[`Cloud Pipelines Scripts`]
...
----
git branch version
git checkout -b cloud-pipelines
----
====
...
The `cloud-pipelines` branch is optional and can be named anything you wish. The intention is for you to use it as a working branch for the changes suggested in this tutorial (hence, you should both create it and check it out).
...
In `$SCP_HOME/credentials`, make two copies of the `$SCP_HOME/concourse/src/pipeline/credentials-sample-cf.yml` file. Rename them as `credentials-fortune-service.yml` and `credentials-greeting-ui.yml`.
...
Edit the Git properties of each credentials file. Make sure to replace the sample values shown in our example. For `concourse-scripts-branch`, you can use a fixed release (use v1.0.0.M8 or later for Cloud Foundry). Leave the other values as they are. We update those in later steps. The following listing shows a credentials file:
...
fly -t myTarget set-pipeline -p fortune-service -c "${SCP_HOME}/concourse/src/pipeline/pipeline.yml" -l "${SCP_HOME}/credentials/credentials-fortune-service.yml" -n
----
====
...
In addition, you should see a new `dev/<app_name>/<version_number>` tag in each GitHub repository and see the app jars uploaded into Bintray.
...
Add a file called `cloud-pipelines.yml` to each application and include the same list of services as in the corresponding `manifest.yml`. Add the necessary details such that Cloud Pipelines can construct a `cf create-service` command.
...
More specifically, for `greeting-ui`, create an `cloud-pipelines.yml` file with the following content:
...
The `cloud-pipelines.yml` file for `fortune-service` is similar, with the addition of the `fortune-db` service, as follows:
...
* A new cloud-pipelines manifest file
...
Cloud Pipelines dynamically creates the services in the test spaces, as defined by the `cloud-pipelines.yml` file we created previously. Optionally, you can add a second section to the `cloud-pipelines.yml` file for the stage environment, and these are created dynamically as well. However, you must always crate prod manually.
...
** Tag the git repositories with `dev/<app_name>/<version_number>` and `prod/<app_name>/<version_number>`.
...
Place this file in the `greeting-ui` repo. The file name and location can be your choice. For this example, we use `cloud-pipelines/manifest-stubrunner.yml`. The following image shows the file in the appropriate folder:
...
Now that we have a jar file and a manifest file for our stub runner application, we need to provide this information to our `greeting-ui` pipeline so that the pipeline downloads the jar from Bintray and deploys it to Cloud Foundry. We do this through the `greeting-ui` `cloud-pipelines.yml` file. We add an entry to the list of services in the `test` section, as follows:
...
----
    - name: stubrunner
      ...
      pathToManifest: cloud-pipelines/manifest-stubrunner.yml
----
====
...
* `pom.xml`
* `cloud-pipelines.yml`
* `cloud-pipelines/manifest-stubrunner.yml`
