...
[source,bash]
----
cf push cloud-pipelines-prometheus -b binary_buildpack -c './prometheus -web.listen-address=:8080' -m 64m
----
====
...
A deployed version for the Cloud Pipelines demo is available https://cloud-pipelines-prometheus.cfapps.io/[here].
...
[source,bash]
----
cf push cloud-pipelines-grafana -b binary_buildpack -c './bin/grafana-server web' -m 64m
----
====

The demo uses Grafana Dashboard with an ID of `2471`.

A deployed version for the Cloud Pipelines demo is available https://cloud-pipelines-grafana.cfapps.io/[here]
